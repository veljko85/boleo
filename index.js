...
var createScene = function () {
  ...
  var boleo180180Arr = [];
  var boleo90180Arr = [];
  var boleoKosaLevaArr = [];
  var boleoKosaDesnaArr = [];
  var allFences = [];
  ...
  function changePosAndScaleFence(valueToCount, activeFence) {
    ...
    if (fencesArr[activeFence].type == "boleo180180") {
  ...
  //MAIN POST MESH //////////////////////////////////////////////////////////////////////////
  BABYLON.SceneLoader.ImportMeshAsync("", "mesh/", "mainPostBoleo.glb").then(
    (result) => {
      ...
      leftWoodPost.material = woodMaterials[0];
  ...
  var createRightFence = (posX, posZ, rotY, type) =>
    BABYLON.SceneLoader.ImportMeshAsync("", "mesh/", "boleoCombo19.glb").then(
      (result) => {
        ...
        function toActiveFence() {
          ...
          if (fencesArr[activeFence].type == "boleo90180")
            setActivnesStyle(changeFence, 0, 1, "active-text-color");
          if (fencesArr[activeFence].type == "boleoKosaDesna")
            setActivnesStyle(changeFence, 0, 2, "active-text-color");
          if (fencesArr[activeFence].type == "boleoKosaLeva")
          ...
          // if (fencesArr[activeFence].color == "larche")
        ...
        //POSTS
        let rightPost = result.meshes[10];
        ...
        //METAL PART TO HOLD WOOD POST
        let rightMetalPart = result.meshes[8];
        rightMetalPart.material = rootMat;
        metalParts.push(rightMetalPart);
        rightMetalParts.push(rightMetalPart);

        //SMALL METAL PART TO HOLD WOOD POST
        let rightSmallMetalPartFront = result.meshes[7];
        let rightSmallMetalPartBack = result.meshes[5];
        let rightSmallMetalPartBoth = result.meshes[6];
        ...
        //WOOD POST
        let rightWoodPost = result.meshes[12];
        ...
        //post roots
        let rightRoot = result.meshes[11];
        if (postType == 0) {
        ...
        foundationRightStart.position = new BABYLON.Vector3(
          getAbsPosX(result.meshes[11]),
          0.0001,
          getAbsPosZ(result.meshes[11])
        );
        ...
        //STRUMANKER
        let rightStrVord = result.meshes[13];
        ...
        ///sturmanker Ruck
        let rightStrRuck = result.meshes[14];
        ...
        rightPostCap.addChild(result.meshes[3]);
        rightPostCap.addChild(result.meshes[4]);
        ...
        let boleo180180 = result.meshes[1];
        boleo180180Arr.push(boleo180180);
        boleo180180.isVisible = false;
        let boleo90180 = result.meshes[4];
        boleo90180Arr.push(boleo90180);
        boleo90180.isVisible = false;
        let boleoKosaDesna = result.meshes[3];
        boleoKosaDesnaArr.push(boleoKosaDesna);
        boleoKosaDesna.isVisible = false;
        let boleoKosaLeva = result.meshes[2];
        boleoKosaLevaArr.push(boleoKosaLeva);
        boleoKosaLeva.isVisible = false;
        ...
        if (type == "boleo180180") boleo180180.isVisible = true;

        if (type == "boleo90180") {
          boleo90180.isVisible = true;
          setSmallerFenceSize(
            rightPost,
            rightPostCap,
            foundationRight,
        ...
        if (type == "boleoKosaLeva") {
          boleoKosaLeva.isVisible = true;
          setSmallerFenceSize(
            rightPost,
            rightPostCap,
            foundationRight,
  ...
  function handleTabActivnes() {
    if (!document.hidden) {
      createRightFence(0.99, 0, 0, "boleo180180");
  ...
  let addNewFenceKosaDesna = document.getElementById("new-fence-kosa-desna");
  addNewFenceKosaDesna.onclick = () => {
    createNewFence(
      createRightFence,
      activeArrowSide,
      rightPosts,
  ...
  let addNewFenceKosaLeva = document.getElementById("new-fence-kosa-leva");
  addNewFenceKosaLeva.onclick = () => {
    createNewFence(
      createRightFence,
      activeArrowSide,
      rightPosts,
    ...
    for (let i = 0; i < fencesArr.length; i++) {
      if (fencesArr[i].type == "boleoKosaDesna") {
        ...
        for (let j = 0; j < fencesArr[i].children.length; j++) {
          if (fencesArr[fencesArr[i].children[j]].type != "boleoKosaLeva") {
  ...
  function changeFenceFunction(c, d, h) {
    ...
    if (d == "boleo180180") {
      boleo180180Arr[h].isVisible = true;
    }
    if (d == "boleo90180") {
      boleo90180Arr[h].isVisible = true;
    }
    if (d == "boleoKosaDesna") {
      boleoKosaDesnaArr[h].isVisible = true;
    }
    if (d == "boleoKosaLeva") {
      boleoKosaLevaArr[h].isVisible = true;
    }
    if (d == "boleoKosaDesna") {
      //children
      let childType = 0;
      for (let i = 0; i < fencesArr[h].children.length; i++) {
        if (fencesArr[fencesArr[h].children[i]].type != "boleoKosaLeva") {
          childType += 1;
        }
      }
      if (childType == 0) {
    ...
    if (d == "boleoKosaLeva") {
      ...
      //parent
      if (h > 0 && fencesArr[h].parent != undefined) {
        if (fencesArr[fencesArr[h].parent].type == "boleoKosaDesna") {
          let fenceSibling = 0;
          for (
            let i = 0;
      ...
      for (let i = 0; i < fencesArr.length; i++) {
        if (
          ...
          fencesArr[i].type != "boleoKosaLeva"
        ) {
          mainPostChildType += 1;
        }
      }
  ...
  changeFence[0].addEventListener("click", () => {
    changeFenceFunction(180, "boleo180180", activeFence);
  });
  changeFence[1].addEventListener("click", () => {
    changeFenceFunction(90, "boleo90180", activeFence);
  });
  changeFence[2].addEventListener("click", () => {
    changeFenceFunction(90, "boleoKosaDesna", activeFence);
  });
  changeFence[3].addEventListener("click", () => {
    changeFenceFunction(90, "boleoKosaLeva", activeFence);
  });
  ...
  //   fencesArr[activeFence].color = "larche";
    ...
    if (fencesArr[activeFence].type == "boleo180180") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(180, "boleo180180", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "boleo90180") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(90, "boleo90180", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "boleoKosaDesna") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(90, "boleoKosaDesna", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "boleoKosaLeva") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(90, "boleoKosaLeva", i);
        }
      }
    }
  ...
  function deleteFence(a) {
    ...
    if (
      ...
      fencesArr[fencesArr[a].parent].type == "boleoKosaDesna"
    ) {
      let childTypee = 0;
